...
class Crawler(object):  
    ...
    def curl(self, url):
        ...
        """
        global urlset
        # To avoid WAFs that prevent bot use
        hdr = {'User-Agent': 'Mozilla/5.0'} 

        # Try to connect, if succeed, save the information about the path on a list
        try:
            # print "retrieving url... [%s] %s" % (self.domain, url)
            req = urllib2.Request('http://%s%s' % (self.domain, url), headers=hdr)
            response = urllib2.urlopen(req)
            if len(url):
                urlSet.append(url.decode('ascii', 'ignore'))
            return response.read().decode('ascii', 'ignore')
        except urllib2.HTTPError, e:
            try:
                # print "retrieving url... [%s] %s" % (self.domain, url)
                req = urllib2.Request('https://%s%s' % (self.domain, url), headers=hdr)
                response = urllib2.urlopen(req)
                if len(url):
                    urlSet.append(url)
                return response.read().decode('ascii', 'ignore')
